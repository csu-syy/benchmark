...
def new_compute_performance_deepgoplus(test_df, go, ont):
...
for types in ['test']:
    ...
    with open('./DataProcess/inputdata/{0}_data_separate.pkl'.format(types), 'rb') as fr:
        real_test_protein_mess = pkl.load(fr)
    ...
    for evas in ['F_max','Smin', 'Aupr', 'ICAupr', 'DPAupr', 'threadhold' ]:
    ...
    with open('./DataProcess/inputdata/all_protein_information.pkl','rb') as fr:
    ...
    for num, tag in enumerate(all_tags):
        ...
        all_proteins = list(pd.read_csv('./DataProcess/inputdata/{0}_data_separate_{1}_proteins.csv'.format(types,tag))['proteins'])
        ...
        for method,mfile in all_files.items():
            ...
            with open(mfile.format(types), 'rb') as fr:
                method_predict_result = pkl.load(fr)
            
            for protein in all_proteins:
                if protein not in method_predict_result:
                    method_predict_result[protein]={}
            ...
            F_max, Smin, Aupr, ICAupr, DPAupr, threadhold = new_compute_performance_deepgoplus(df,go,tag)
            ...
            all_results["{0}_{1}".format('threadhold', num)].append(round(threadhold,5))

            print('Have done', method, tag, 'F_max:', F_max, 'Smin', Smin, 'Aupr:', Aupr,'ICAupr', ICAupr, 'DPAupr', DPAupr, 'threadhold:',threadhold)
    
    df = pd.DataFrame(all_results)
    df.to_csv('./result/{0}_evaluation_results_run_codes5.csv'.format(types))
